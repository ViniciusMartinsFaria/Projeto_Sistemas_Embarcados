library ieee;
use ieee.std_logic_1164.all;
use ieee.numeric_std.all;
use std.textio.all;             -- manipular arquivos arquivos
use ieee.std_logic_textio.all;  -- ler Hex
use work.array_bidimensional.all;

entity tb_sobel is
end tb_sobel;

architecture sim of tb_sobel is
    
    -- Definição da imagem completa na memória do TB
    type t_memoria_img is array (0 to 63, 0 to 63) of std_logic_vector(7 downto 0);
    signal img_rom : t_memoria_img;
    type t_memoria_res is array (0 to 63, 0 to 63) of integer range 0 to 255;
    shared variable matriz_saida : t_memoria_res := (others => (others => 0));

    signal  clock_sg    : std_logic := '0';
    signal  reset_sg    : std_logic := '1';
    signal  bloco_sg    : matriz;
    signal  pixel_sg    : std_logic_vector(7 downto 0);

    component sobel_topo is
    port (
        clock               : in std_logic;
        reset               : in std_logic;
        bloco               : in matriz;
        pixel               : out std_logic_vector(7 downto 0));
    end component;

begin

    inst_Sobel: sobel_topo
    port map (
        clock   => clock_sg,
        reset   => reset_sg,
        bloco   => bloco_sg,
        pixel   => pixel_sg
    );

    clock_sg <= not clock_sg after 10 ns;

    -- PROCESSO PRINCIPAL
    process
        file arquivo_entrada : text open read_mode is "C:\Users\Asus\Desktop\Kernels\imagem_matriz_hex.txt"; -- Seu arquivo de entrada
        file arquivo_saida   : text open write_mode is "C:\Users\Asus\Desktop\Kernels\resultado_sobel.txt";   -- O arquivo que vamos criar
        
        variable linha_texto : line;
        variable temp_byte   : std_logic_vector(7 downto 0);
        variable v_pixel_int : integer;
        variable v_pixel_sat : integer; -- Valor saturado (0-255)
    begin
        -- 1. CARREGAR A IMAGEM DE ENTRADA NA MEMÓRIA ROM
        report "Carregando imagem de entrada...";
        for y in 0 to 63 loop
            readline(arquivo_entrada, linha_texto);
            for x in 0 to 63 loop
                hread(linha_texto, temp_byte);
                img_rom(y, x) <= temp_byte;
            end loop;
        end loop;

        wait for 20 ns;
        reset_sg <= '0';
        wait for 20 ns;

        -- 2. PROCESSAMENTO (Hardware Sobel)
        report "Iniciando processamento Sobel...";
        
        -- Loop de 0 a 61 (Janela deslizante)
        -- O pixel resultante corresponde à posição (y+1, x+1) da imagem original
        for y in 0 to 61 loop
            for x in 0 to 61 loop
                
                -- Alimenta a entrada 'bloco' com os dados da memória
                -- (Mesma lógica de antes)
                bloco_sg(0,0) <= img_rom(y,   x);
                bloco_sg(0,1) <= img_rom(y,   x+1);
                bloco_sg(0,2) <= img_rom(y,   x+2);
                
                bloco_sg(1,0) <= img_rom(y+1, x);
                bloco_sg(1,1) <= img_rom(y+1, x+1);
                bloco_sg(1,2) <= img_rom(y+1, x+2);
                
                bloco_sg(2,0) <= img_rom(y+2, x);
                bloco_sg(2,1) <= img_rom(y+2, x+1);
                bloco_sg(2,2) <= img_rom(y+2, x+2);

                -- Aguarda o clock para o hardware processar
                wait until rising_edge(clock_sg);
                
                -- Aguarda um delta time para o sinal 's_pixel' atualizar após o clock
                wait for 1 ns; 

                -- 3. CAPTURA E SATURAÇÃO DO RESULTADO
                -- Converte std_logic_vector(16 bits) para integer
                v_pixel_int := to_integer(unsigned(pixel_sg));

                -- Lógica de Saturação: Se for maior que 255 (branco estourado), trava em 255
                --if v_pixel_int > 255 then
                    --v_pixel_sat := 255;
                --else
                    --v_pixel_sat := v_pixel_int;
                --end if;

                -- Salva na nossa matriz buffer (Note o índice +1 para centralizar e preservar bordas)
                matriz_saida(y+1, x+1) := v_pixel_int;
                
            end loop;
        end loop;

        -- 4. ESCRITA DO ARQUIVO FINAL (DUMP DA MATRIZ)
        report "Escrevendo arquivo de saída: resultado_sobel.txt";
        
        for y in 0 to 63 loop
            for x in 0 to 63 loop
                -- Escreve o valor em Hexadecimal no arquivo
                hwrite(linha_texto, std_logic_vector(to_unsigned(matriz_saida(y,x), 8)));
                
                -- Adiciona um espaço entre os números (exceto no último)
                if x < 63 then
                    write(linha_texto, string'(" "));
                end if;
            end loop;
            writeline(arquivo_saida, linha_texto);
        end loop;

        report "Simulação e Geração de Arquivo Concluídas com Sucesso!";
        wait; -- Para a simulação
    end process;

end sim;